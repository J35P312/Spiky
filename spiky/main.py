...
def main():
    ...
    # Mutually exclusive module selection
    module_group = parser.add_mutually_exclusive_group(required=True)
    module_group.add_argument("--coverage-bed", action="store_true", help="Generate windowed coverage BED from BAM")
    module_group.add_argument("--predict-sex", action="store_true", help="Predict sample sex from coverage BED")
    module_group.add_argument("--generate-regions", action="store_true", help="Generate robust chrY regions from multiple predict-sex outputs")
    module_group.add_argument("--model", action="store_true", help="Train linear regression model for FFY from coverage BEDs")
    module_group.add_argument("--predict", action="store_true", help="Predict FFY for a coverage BED using a trained model")
    module_group.add_argument("--version", action="store_true", help="Print the version of the tool")
    ...
    parser.add_argument("--female-median-threshold", type=float, default=0.01, help="Maximum median chrY fraction for females to consider a bin usable")
    ...
    if args.coverage_bed:
        ...
        generate_regions.run(
            ...
            female_median_threshold=args.female_median_threshold,
            out_bed=sys.stdout,
